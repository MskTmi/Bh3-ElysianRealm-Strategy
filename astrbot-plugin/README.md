...
- ⭐ **多流派支持**：同一角色的不同流派（普攻流、大招流等）可共享关键词，自动返回最新版本
...
- 发送 `猫猫乐土` 获取帕朵菲利丝攻略（如有多个流派，返回最新更新的）
- 发送 `人律乐土` 获取人之律者攻略
- 发送 `大鸭鸭乐土` 获取次生银翼攻略
- 发送 `最新攻略` 或 `最新乐土攻略` 或 `最新乐土` 获取最近更新的攻略（**新功能**）

**多流派支持**：当一个角色有多个流派（如普攻流、大招流）时，可以：
- 使用共享关键词（如 `猫猫乐土`）：自动返回最新更新的流派
- 使用特定关键词（如 `猫猫普攻流`）：返回指定流派的攻略
...
4. **多流派智能选择**：同一角色的多个流派可共享关键词，自动返回最新更新的版本
...
假设猫猫有两个流派的攻略：
- `Felis_Attack.png` (普攻流，更新于 2024-01-10)
- `Felis_Ultimate.png` (大招流，更新于 2024-01-15)
...
Felis_Attack:
  keywords:
    - 猫猫乐土
    - 猫猫普攻流
  last_updated: '2024-01-10T10:00:00+00:00'
...
用户行为：
- 发送 `猫猫乐土` → 返回 Felis_Ultimate（更新时间更晚）
- 发送 `猫猫普攻流` → 返回 Felis_Attack（特定流派）
- 发送 `猫猫大招流` → 返回 Felis_Ultimate（特定流派）

**示例**：
```
用户: /更新乐土攻略
机器人: 乐土攻略更新完成
       更新的角色: Felis_Ultimate, Human
...
机器人: 最新更新的攻略：猫猫乐土 (更新于 2024-01-15)
       [发送 Felis_Ultimate 攻略图片]

用户: 猫猫乐土
机器人: [发送 Felis_Ultimate 攻略图片]  ← 自动选择最新的流派
```
